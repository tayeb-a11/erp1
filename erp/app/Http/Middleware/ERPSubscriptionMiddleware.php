...
class ERPSubscriptionMiddleware
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (!$subdomain) {
            Log::warning('ERPSubscriptionMiddleware: Impossible d\'extraire le subdomain', [
        ...
        if ($subscriptionStatus === null) {
            Log::error('ERPSubscriptionMiddleware: Erreur lors de la vérification de l\'abonnement', [
                'subdomain' => $subdomain,
                'url' => $request->fullUrl()
            ]);
            return $next($request);
        ...
        if ($subscriptionStatus['is_expired']) {
            Log::info('ERPSubscriptionMiddleware: Abonnement expiré, redirection', [
                'subdomain' => $subdomain,
    ...
    private function checkSubscriptionFromGlobalAPI(string $subdomain): ?array
    {
        try {
            ...
            Log::error('ERPSubscriptionMiddleware: Réponse API invalide', [
                'subdomain' => $subdomain,
            ...
            Log::error('ERPSubscriptionMiddleware: Exception lors de l\'appel API', [
                'subdomain' => $subdomain,
                'error' => $e->getMessage()
            ]);
        }
